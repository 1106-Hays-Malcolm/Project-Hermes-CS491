...
# This version features a timer for metrics
...
# This controls how often inference occurs. It will be paused in the next version
interval = 1
...
#current bool val is inside function for determining intervals but will be moved outside when logic changes due to combining
#Timing mechenism also need its own function
def run_inference(model, processor, device):
     while True:
        if True:
            t1 = time.time()
...
def main():
    ...
    run_inference(model, processor, device)
